...
const PartnersMainContent = () => {
  ...
  const fetchePartners = async () => {
    const response = await fetch("https://api.quickt.com.au/api/partners?populate=*", {
      headers: {
  ...
  const allPartners = Array.isArray(partners)
    ? partners?.map((item) => ({
      ...
      first_name: item.attributes?.users_permissions_users?.data[0]?.attributes?.first_name,
      last_name: item.attributes?.users_permissions_users?.data[0]?.attributes?.last_name,
      userimage: item.attributes?.users_permissions_users?.data[0]?.attributes?.image,
    }))
    : [];
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div style={{ display: "flex", alignItems: "center" }}>
          {
            params.row.userimage && <img
              src={params.row.userimage} // Access the image property from the details object
              alt="User"
              style={{
                borderRadius: "50%",
                marginRight: "8px",
          ...
          <div>
            <p style={{ marginTop: "20px" }}>{params.row.first_name} {params.row.last_name}</p>
            <br />
